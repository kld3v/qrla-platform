	...
	<div
		...
		<v-sheet v-if="totalAttachment != 0">
			<Link
				...
				:key="chat.id">
				<div
					class="d-flex gap-3 align-center mt-5 attachment"
					v-for="atch in chat.attachment"
					:key="atch.file">
					<v-avatar
						class="rounded-md bg-grey100"
						size="48">
						<img
							width="24"
							alt="file"
					...
					<DownloadIcon
						class="ms-auto download textPrimary hover-link-primary"
						size="20"></DownloadIcon>
				</div> </Link
			>href
		</v-sheet>
		<v-sheet v-else>
